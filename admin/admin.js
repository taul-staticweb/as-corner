document.addEventListener('DOMContentLoaded', () => {
    let GAS_URL = sessionStorage.getItem('admin_gas_url');
    let quill;
    ...
    btnLogin.addEventListener('click', async () => {
        const url = inputApiKey.value.trim();
        if (!url) return;
        ...
        try {
            const response = await fetch(`${url}?type=all&v=${Date.now()}`);
            if (!response.ok) throw new Error('Gagal');
            const data = await response.json();
            
            GAS_URL = url;
            sessionStorage.setItem('admin_gas_url', GAS_URL);
            loginOverlay.style.display = 'none';
    ...
    inputApiKey.addEventListener('keyup', (e) => {
        if (e.key === 'Enter') {
            btnLogin.click();
        }
    });
